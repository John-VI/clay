...
int main(int argc, char *argv[]) {
  ...
  sdlerr = !SDL_CreateWindowAndRenderer(progtitle, 800, 600, 0, &win, &ren);
  if (sdlerr) {
    fprintf(stderr, "Birth crash!\nSDL_GetError():\n%s\n", SDL_GetError());
    SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_ERROR, "Clay Fatal Error!",
                             SDL_GetError(), NULL);
  ...
  while (1) {
    while (SDL_PollEvent(&e)) {
      if (e.type == SDL_EVENT_QUIT) {
        goto main_loopescape;
      } else if (e.type == SDL_EVENT_KEY_DOWN && menup()) {
        menuevent(e.key.scancode);
      }
    }
    ...
    if (kbd[SDL_SCANCODE_ESCAPE] && !menup())
      initmenu(0);
    else if (!menup()) {
      if (kbd[SDL_SCANCODE_SPACE])
        ...
        int i;
        for (i = 24;
             ((*(uint32 *)(kbd + SDL_SCANCODE_RIGHT)) >> i & 255) == 0 &&
             i >= 0;
             i -= 8)
          ;
        bgstate = (i / 8) + 1;
      ...
      while (ccons) {
        ...
        if (fly->x < 0.0f || fly->x > 800.0f || fly->y < 0.0f ||
            fly->y > 600.0f) {
